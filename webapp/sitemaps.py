...
The static pages under /careers have no dedicated route here -- they're
discovered by the generic directory_parser-based /sitemap_tree.xml (see
webapp/app.py:build_sitemap_tree) instead, now that its lastmod dates
work (patched below). /partners keeps its own dedicated route, but its
page list and lastmod dates are still generated by directory_parser
