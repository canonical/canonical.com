...
def convert_pull_quotes(content_html):
    ...
    and placed below the quote. The macro is designed for full-width page
    ...
    for blockquote in blockquotes:
        ...
        # so the quote spans the full body width.
        ...
        if content_col is not None:
            ...
            if divider is not None:
                divider.decompose()
            # Keep the original citation, displayed below the quote, muted to
            # match the vf_quote_wrapper citation styling.
...
class CanonicalBlogViews(BlogViews):
    # Number of related articles to show at the bottom of an article page.
    # Overrides the upstream default of 3 so the related posts fill the
    # vf_blog 4-column layout.
    ...
    def _get_article_context(
        ...
        content = context.get("article", {}).get("content", {})
        if content.get("rendered"):
            content["rendered"] = convert_pull_quotes(content["rendered"])

        # Upstream caps related_articles at 3; re-fetch enough to fill the
        # 4-column layout, keeping the same compatibility-based ordering.
