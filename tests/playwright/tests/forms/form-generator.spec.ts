...
test.describe("Modal interaction tests", () => {
  ...
  test("should close the modal with close button", async ({ page }) => {
    ...
    const closeButton = modal.locator(
      'button[aria-label="Close active modal"]'
    );
  ...
  test("should restore focus to trigger element after closing", async ({
    page,
  }) => {
    await openModal(page);
    const modal = page.locator("#contact-modal");
...
test.describe("Modal validation tests", () => {
  ...
  test("should disable submit button when required checkbox is not checked", async ({
    page,
  }) => {
  ...
  test("should enable submit button when required checkbox is checked", async ({
    page,
  }) => {
    // Check the required checkbox
    await page
      .locator('input[aria-labelledby="Physical server"]')
      .check({ force: true });
  ...
  test("should fill and redirect to marketo submission endpoint", async ({
    page,
  }) => {
  ...
  test("should return 400 error when honeypot is triggered", async ({
    page,
  }) => {
  ...
  test("should show textbox when 'Other' checkbox is checked", async ({
    page,
  }) => {
...
test("should retain form data when modal is closed and reopened", async ({
  page,
}) => {
  // Fill some fields
  const testFields = [
