...
function initModals(modalId, ariaControls, open) {
    ...
    if (modal && modal.classList.contains("p-modal")) {
      ...
      if (open) {
        ...
        if (focusAfterClose && focusAfterClose.focus) {
          ...
          // Reset to null so a stale reference cannot scroll the page on
          // future (spurious) closeModals() calls after the modal is closed.
  ...
  document.addEventListener("click", function (e) {
    // GUARD: only act when the modal is actually visible.
    //
    // Without this guard, closeModals() fires on EVERY click on the page.
    // After the modal has been opened once, `focusAfterClose` holds a
    // reference to the trigger element (the "Export to PDF" button). Every
    // subsequent closeModals() call would invoke focusAfterClose.focus(),
    // which scrolls the page to that button — making every click appear to
